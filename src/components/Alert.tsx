...
const Alert: FC<AlertProps> = ({ message, type, counter }) => {
  ...
  const displayMessage = async (alertMessage: string) => {
    ...
    if (type === AlertTypes.SUCCESS) {
      alertMessageElement.style.backgroundColor = "green";
    } else if (type === AlertTypes.UPDATE) {
      alertMessageElement.style.backgroundColor = "purple";
    } else if (type === AlertTypes.ERROR) {
      alertMessageElement.style.backgroundColor = "red";
    }
