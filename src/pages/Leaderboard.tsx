import { default as axios, default as get } from "axios";
  ...
  const toggleOverlay: (
    ...
    if (cardStyles.display === "none") {
      tr!
        .querySelectorAll("td")
        .forEach((item) =>
          item.getAttribute("data-key") === "username"
            ? (targetUsername = item.textContent!)
            : null
        );

      (card.querySelector(".title h2") as HTMLElement).textContent = `Follow ${
      ...
      if (targetUsername) {
        try {
          ...
          if (e instanceof axios.AxiosError) {
            (
              document.getElementById("errorMessage") as HTMLElement
            ).textContent = e.response!.data.error;
          }
        }
      }
    }
  ...
  return (
    <Sidebar
      mainContent={
        <>
          {showAlert ? (
            <Alert message="Success" type={AlertTypes.SUCCESS} counter={0} />
          ) : null}
                  ...
                  <tbody>
                    {Object.entries(leaderboard).map(([index, data]) => (
                      <tr key={index}>
                        ...
                        <td data-key="earnings" style={{ color: "#44c809" }}>
                          {data["earnings"]}
                        </td>
                        <td>
                          <button
